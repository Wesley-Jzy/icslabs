...
## 1. Add iaddl
## 2. design:  W-iaddl/opl M:mrmovl E-no cc D-jle F:... for the 100% jle 
...
int f_pc = [
	# cc/mrmovl/not opl iaddl/jle
	M_icode == IMRMOVL && !(E_icode in { IOPL, IIADDL }) &&
	(D_icode == IJXX && f_ifun == 1) && !e_Cnd : D_valP; 
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) &&
	!(W_icode == IMRMOVL && !(M_icode in { IOPL, IIADDL }) &&
	(E_icode == IJXX && E_ifun == 1)) ||
	# Stalling at fetch while ret passes through pipeline
