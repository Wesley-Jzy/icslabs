...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch(icode)
    {
        ...
        case I_PUSHL:
        case I_POPL:
            if (!get_byte_val(sim->m, next_pc, &rA_rB)) 
            {
                err_print("PC = 0x%x, Invalid instruction address", sim->pc);
                return STAT_ADR;
            }
    ...
    switch (icode) 
    {
        ...
        case I_RRMOVL:  /* 2:x regA:regB */
            if (!NORM_COND((cond_t)ifun))
            {
                err_print("PC = 0x%x, Invalid ifun", sim->pc);
                return STAT_INS;
            }

            if (!NORM_REG(rA) || !NORM_REG(rB))
            {
                err_print("PC = 0x%x, Invalid register", sim->pc);
                return STAT_INS;
            }
